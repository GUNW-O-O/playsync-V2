...
이 API가 하지 않기로 한 부분 성공이다. 검사는 같은 `FOR UPDATE`로 직렬화되도록
트랜잭션 안, 검사 2 옆에 둔다(검사 3).
...
있어서 뒤이은 해제의 비트 0보다 늦게 도착하는 경우 — 는 그대로 남는다. 그건
T28이 입장의 레디스 쓰기를 락 밖에 둔 데서 오는 것이고 T29 이전부터 있었다.
여기서 고치지 않는다.
