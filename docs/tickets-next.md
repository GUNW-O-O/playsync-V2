...
**항목**: T41이 남긴 "외삽 상한 약 1,200테이블"을 실측으로 좁힌다.
**범위**: `load/scenarios/ramp.js` · `load/lib/table.js` ·
`backend/docker-compose.test.yml`의 k6 서비스 · 결과 문서.
**프론트 영향**: 없다. 제품 코드는 한 줄도 안 건드렸다.

수치는 [`results/2026-08-17-load-ceiling.md`](./results/2026-08-17-load-ceiling.md)에
있다. 여기에는 **판단**만 적는다.

### 재는 대상을 두 번 바꿨다

처음 계획은 단순했다 — 지난번 상한(66테이블)을 792까지 밀어 터지는 지점을 본다.
그런데 두 번 막혔고, 그때마다 **막은 것이 서버가 아니었다.**

**첫 번째는 측정기가 서버를 굶겼다.** VU 660개로 올리자 k6가 코어 19개를 먹었고
서버는 12테이블에서 붕괴 판정으로 죽었다 — 그 순간 서버 CPU는 10%였다. 원인이
`ramp.js`가 매니페스트를 **VU마다** 파싱하는 것이었는데, 이건 T40부터 있던
코드다. 330 VU에서는 안 아프고 660에서 아픈 비용이라 **지난 실행이 통과한 것도
맞고 지금 통과 못 한 것도 맞다.** `SharedArray`로 고쳤다.

여기서 compose의 판단 하나를 뒤집었다. k6에 CPU 상한을 걸지 않은 근거가
"측정기가 피측정자를 굶기면 재는 것이 서버가 아니라 측정기가 된다"였는데,
**정확히 반대 방향으로 같은 일이 났다** — 호스트가 포화되면 백엔드의 `cpus: 1`은
상한일 뿐 하한이 아니다. 그래서 k6에도 상한을 걸되 넉넉하게(10코어) 준다.
원래 근거는 살아 있으므로 **k6가 자기 상한에 붙으면 그 실행의 수치는 서버의 것이
아니다**를 주석에 남겼다.

**두 번째는 도착이 정원을 가렸다.** 1코어에서 착석은 초당 5~6명이 한계라
594테이블을 앉히는 데만 18분이 걸리고, 그동안 코어가 꽉 차 고원 수치에 도착
잔업이 섞인다. 실제로 실행 A의 462테이블 고원(CPU 38%)이 나중에 잰 594테이블
고원(CPU 33%)보다 **테이블이 적은데 더 높았다.**

그래서 **착석 구간에만 `docker update --cpus`로 코어를 빌려주고 고원 직전에
1코어로 되돌리는** 절차를 만들었다. 이게 측정을 무르게 만들지 않는다고 판단한
근거는 둘이다. 착석 몰림은 **실행 A가 이미 1코어로 따로 쟀고**(초당 5~6명,
480테이블에서 불편선), 시드가 대회 생성을 측정 밖에 두는 것과 같은 자리다 —
실제로도 손님은 대회 직전에 몰려 앉지, 서버가 대회 내내 그 속도로 사람을 받지
않는다.

### 코어를 한 번에 떨구면 그건 포화가 아니라 충격이다

660테이블에서 6코어를 1코어로 한 번에 떨구자 lag p95가 203ms로 붕괴선을 넘었다.
처음에는 "660이 한계"로 읽었는데, 594를 6→2→1로 나눠 되돌리니 **CPU 33% ·
lag 1ms로 완주**했다. 착석 잔업이 얹힌 순간을 포화로 읽을 뻔했다.

그래서 결과 문서에 660과 594의 차이를 **규모 차이로 적지 않았다.** 594는
실측이고, 진짜 경계는 이 기계에서 못 잰다.

### 소켓 대신 액션으로 태운 이유

이 기계는 VU당 소켓 열 개라 660테이블(소켓 6,600)에서 k6가 Docker VM 메모리를
채운다. 테이블 수를 더 못 올리므로 **테이블당 액션 속도**를 올려 같은 부하를
만들었다 — 330테이블 × 8배면 서버가 처리하는 액션은 2,600테이블어치다.

여기서도 한 번 틀렸다. 압축값을 **처음부터** 걸었더니 95테이블에서 죽었는데,
그건 처리 정원이 아니라 착석과 게임이 같은 코어를 두고 싸운 결과다. 무대가 다
선 뒤에 속도를 올리도록 `LOAD_BURN_AFTER_S`를 넣었다.

이 축이 답한 것은 정원과 다른 질문이다. **초당 몇 액션까지 처리하나** — 약
2,600테이블어치에서 내 액션 p95가 합격선(200ms)을 넘는다. 그런데 그때도 붕괴선은
안 닿았다(lag 최대 38ms). 이 서버가 포화되는 방식은 이벤트 루프가 막히는 것이
아니라 CPU가 차면서 왕복이 길어지는 것이다.

### 무대가 답을 제한한다는 것도 결과다

호스트 RAM이 7.9GB(여유 1GB)라 Docker VM이 3.785GB이고, k6가 VU당 3~4.5MB를
쓴다. **600 VU 부근이 이 기계의 측정 상한**이고, 그 위에서는 서버가 멀쩡한데
측정값만 무너진다(실행 D — 서버 lag 9.8ms인데 내 액션 4초).

그래서 이번 문서의 상한(메모리 기준 약 1,400테이블)도 여전히 외삽이다. 실측은
594까지다. 이걸 "1,400테이블 됩니다"로 적지 않고 **어디까지가 실측이고 어디부터
계산인지**를 표로 갈라 놓았다.

### 남긴 것

- `rebuysAccepted`가 여섯 번 실행에서 전부 0이다. 지난 실측도 그랬다. 봇이
  즉시 수락하도록 짰는데 경로가 안 도는 것인지 탈락이 안 나오는 것인지 안
  갈렸다.
- `socket_errors`가 실행마다 1~12건이다. 지난 실측(66테이블)에서는 0이었다.
...
항목(`dealerToken` 우선순위)을 같은 티켓에서 닫는다.
**범위**: `lib/token-cookie.ts`(신규) · `auth/action.ts` ·
`(terminal)/dealer/action.ts` · `(terminal)/table/action.ts` · 그 스펙들.
**백엔드 영향**: 없다.
**끝났다고 하는 기준**: 쿠키가 토큰보다 먼저 죽지도, 오래 남지도 않는다.

### 문제 — T43이 고친 것이 프론트에서 되살아나 있었다

T43이 토큰 수명을 역할별로 갈랐다(좌석·딜러·상점 콘솔 12시간, 그 밖 1시간).
**프론트 쿠키는 따라가지 않았다.** 세 곳이 각자 숫자를 적고 있었다.

| 쿠키 | 쿠키 `maxAge` | 실제 토큰 수명 | 결과 |
|---|---|---|---|
| `dealerToken` | 1시간 | 12시간 | 토큰이 11시간 더 살아 있는데 태블릿이 자격을 잃는다 |
| 좌석 토큰(`accessToken`) | 1시간 | 12시간 | **좌석마다** 한 시간에 한 번 OTP 재입력 |
| 로그인 `accessToken` | 1일 | 1시간(USER) | 죽은 토큰을 23시간 들고 다니며 401 |

가운데 줄이 정확히 **T43이 없애려던 증상**이다 — 티켓 문서에 "네 시간짜리
대회면 전원이 겪는다"고 적어 둔 그것이, 상수를 백엔드에서만 고쳐서 프론트에
그대로 남아 있었다. 주석까지 `백엔드 JWT 만료가 1시간이다`로 남아 **문서가
거짓을 말하고 있었다.**

### 값을 맞추지 않고 출처를 하나로

숫자를 12시간으로 고쳐 적는 것이 제일 쉬운 수정인데, 그러면 **다음에 백엔드가
바꿀 때 같은 일이 또 난다.** 어긋난 것을 잡아 주는 장치가 없다 — 타입 체커도
CI도 두 숫자를 대조하지 않는다. 이 리포가 줄 번호 대신 이름으로 코드를
가리키는 이유와 같은 자리다.

그래서 `cookieMaxAgeFromToken`이 **토큰의 `exp`에서 남은 초를 뽑는다.**
백엔드가 수명을 바꾸면 쿠키가 저절로 따라가고, 역할마다 다른 수명도 분기 없이
맞는다.

**서명은 검증하지 않는다.** 방금 우리 백엔드가 준 토큰이고, 여기서 정하는
것은 권한이 아니라 브라우저에 얼마나 보관할까뿐이다. 위조된 `exp`로 얻는 것은
자기 쿠키를 일찍 버리거나 오래 들고 있는 것뿐이고, 진짜 만료는 백엔드가 매
요청 검증한다.

읽을 수 없으면 기본값(1시간)으로 떨어진다. 여기서 던지면 로그인·입장이 통째로
실패하는데 **우리가 못 읽는 것과 토큰이 나쁜 것은 다르다.** 반대로 이미 만료된
토큰은 `0`을 준다 — 브라우저가 즉시 버리므로 다음 요청이 "토큰 없음"으로
깨끗하게 갈린다. 살아 있는 척하는 쿠키를 들고 401을 받는 것보다 낫다.

### 같이 닫은 것 — 반대편 쿠키를 지운다

수명만 고치면 **T24 이월 항목이 더 나빠진다.** `api/ws-ticket/route.ts`와
좌석 페이지가 `dealerToken`을 먼저 보는데 지우는 경로가 없어서, 딜러로 쓰던
태블릿을 좌석으로 돌리면 손님의 좌석 토큰 대신 옛 딜러 토큰으로 티켓이 나가고
게이트웨이가 `payload.tableId !== tableId`로 1008을 던진다. 그 창이 쿠키
만료까지인데, 수명을 맞추면 **1시간에서 12시간이 된다.**

우선순위를 뒤집는 대신 **인증 성공 시 반대편을 지운다** —
`authenticateDealer`는 `accessToken`을, `enterSeat`는 `dealerToken`을.
근거는 물리다: **태블릿 하나는 한 번에 한 자리**이고, 딜러 태블릿이 고장 나면
좌석 태블릿을 딜러용으로 돌려 쓰는 일이 실제로 있다. 마지막에 인증한 역할이
그 기기의 역할이다.
...
| 검사 | expected / received |
|---|---|
| `enterSeat`가 쿠키 수명을 `exp`에서 뽑는다 | `maxAge: 43200` / `maxAge: 3600` |
| `enterSeat`가 옛 딜러 토큰을 지운다 | `delete('dealerToken')` 호출 / 없음 |
| `authenticateDealer`가 쿠키 수명을 `exp`에서 뽑는다 | `maxAge: 43200` / `maxAge: 3600` |
| `authenticateDealer`가 좌석·사용자 토큰을 지운다 | `delete('accessToken')` 호출 / 없음 |

`cookieMaxAgeFromToken` 자체는 신규 함수라 파일 없음으로 빨갰다(9건).

### e2e에 넣지 않은 이유

"딜러였던 태블릿을 좌석으로 되돌린다"는 상태 전이라 e2e의 재료인데,
`stage()` 하나가 브라우저 컨텍스트 하나라 **한 컨텍스트에서 역할을 갈아 끼우는
장면이 지금 무대에 없다.** 무대를 늘리면 촬영 장면 순서까지 건드리게 되므로,
쿠키 조작 자체는 서버 액션 단위 스펙으로 못 박고 여기 적어 둔다.

### 기준선

프론트 단위 100 → **114**(24 → **26** files). 백엔드는 손대지 않아 단위 208 ·
통합 403 그대로이고, e2e 13 · 타입 에러 0도 그대로다.

---

## T53 — 인증 없는 라우트의 요청율 상한

**항목**: `backlog.md`의 T23 이월 항목 둘("대회 단위 잠금이 DoS 원시함수다",
"게이트가 bcrypt 앞이라 정상 로그인도 슬롯을 쓴다").
**범위**: `auth/throttle.ts`(신규) · `app.module.ts` · `auth.controller.ts` ·
`dealer.controller.ts` · `dealer/otp-attempts.ts` · `dealer.service.ts` ·
`backend/.env.example` · `docker-compose.test.yml`.
**프론트 영향**: 없다. 정상 사용은 상한에 닿지 않는다.
**끝났다고 하는 기준**: 인증 없는 라우트를 무한정 때릴 수 없고, OTP를 맞힌
요청이 잠금 카운터에 흔적을 남기지 않는다.

### 문제 — 잠금이 무기가 된다

T23이 딜러 OTP에 대회 단위 잠금을 걸었다(5회 / 5분). 추측은 그걸로 막힌다.
문제는 **그 잠금을 거는 일이 공짜**라는 것이다. `POST /dealer/auth`에 인증이
없으므로 같은 망의 누구나 틀린 OTP 다섯 개로 그 대회의 신규 딜러 로그인을
닫을 수 있고, 상점이 재발급으로 풀어도 다섯 번 더 보내 다시 닫는다.

**`POST /auth/login`은 잠금 장치조차 없다.** 인증이 없고 bcrypt가 돈다
(T41 실측 p50 58ms). 스레드풀을 직접 태울 수 있는 문이 열려 있었다.

### 범위를 줄인 판단

착수 전 계획은 이월 항목이 적어 둔 대로 **IP 단위 카운터 + 전역 Throttler
둘 다**였다. 설계하다 앞의 것을 뺐다.

- Throttler가 이미 IP 기준이라 **가해자 쪽 비용은 그것으로 붙는다.**
- 그 위에 카운터를 더해도 주소 하나가 잠글 수 있는 대회 수가 20개에서 4개로
  주는 정도다. 주소를 바꾸면 둘 다 무력이라는 것도 같다.
- 대신 **프록시 뒤 IP 판별**이라는 함정을 직접 떠안는다. 잘못 읽으면 손님
  한 명의 오타가 전원의 카운터를 올린다 — 방어 장치가 정확히 반대로 도는
  모양이다.

표준 부품 하나로 끝나는 자리에 손으로 만든 부품을 더할 값이 없다.

**선을 어디에 긋는지도 같이 적었다**(`throttle.ts`). 주소를 바꿔 가며 오는
공격은 막지 않는다. 이 도메인의 신뢰 경계는 행사장 폐쇄망이고 현장에 직원이
있다. 자동화된 폭주를 자르는 선에서 멈춘다 — 상상한 공격을 하나씩 막다 보면
끝이 없고, 실제로 뚫리는 날 그 모양을 보고 고치는 편이 싸다.

### 이 토폴로지에서 IP 하나가 뜻하는 것

브라우저는 백엔드를 직접 부르지 않는다. 서버 액션·서버 컴포넌트는
`BACKEND_URL` 직통이고 클라이언트 fetch는 Next rewrite를 탄다 — **어느 쪽이든
백엔드가 보는 주소는 Next 프로세스 하나**다. 그래서 전역 한도는 "단말 하나의
몫"이 아니라 **모든 사용자 트래픽의 몫**으로 잡아야 한다(전광판만 해도 1초
폴링이다). 값이 실제로 값을 내는 자리는 백엔드에 직접 붙는 스크립트 쪽이다.

단말별로 가르려면 프론트가 싣는 `X-Forwarded-For`를 믿어야 하는데, 그 헤더는
직접 붙는 쪽도 위조할 수 있어 "프론트에서 온 연결일 때만 믿는다"는 판별이
따로 필요하다. 하지 않았다 — 얻는 것이 버킷 세분화 하나뿐이고, 잘못 켜면
아무나 남의 버킷을 채운다.
...
| | 값 | 왜 |
|---|---|---|
| 창 | 60초 | 짧으면 버스트를, 길면 지속 도착을 잡는다. 중간 |
| 전역 | 600 / 분 | Next 하나가 전체를 중계한다는 전제. 전광판 폴링(분당 60)에 조회를 더해도 남는다 |
| 인증 라우트 | 120 / 분 | bcrypt가 도는 자리라 좁게. 초당 2 |

부하 무대만 env로 올린다(`docker-compose.test.yml`의 `backend-load`). k6가
컨테이너 하나라 **모든 가상 사용자가 한 주소에서 온다** — 기본값이면 램프가
자기 자신에게 막힌다.

**끄는 스위치를 두지 않았다.** 부하에서도 제한 코드가 그대로 실행돼야 "켜면
무너지는" 상태가 안 생긴다. 기본값을 방어값으로 둔 것도 같은 이유다 — 반대로
두면(기본 느슨 + 운영에서만 조임) 실제로 도는 경로는 늘 느슨한 쪽이고 조인
설정은 아무도 밟지 않은 코드가 된다. T51에서 `createTestPrisma()`가 제품과
다른 클라이언트라 통합 394건이 제품 설정을 한 번도 안 밟았던 것과 같은 함정이다.

`positiveInt`가 빈 값·문자·0·음수를 전부 기본값으로 되돌린다. `Number('')`는
0이고 `Number('abc')`는 NaN인데, **0은 모든 요청을 막고 NaN은 아무도 안
막는다.** 오타 하나가 어느 쪽으로든 조용히 무너뜨리는 자리다.

### 슬롯 반납 — 안내가 거짓말을 하고 있었다

게이트가 bcrypt **앞**이라 정답도 슬롯을 하나 쓴다. 그래서 OTP는 맞았는데
다른 이유로 막히는 경로(닫힌 대회 · 딜러 세션 없음 · 남의 테이블)가 카운터를
채웠고, 여섯 번째 재시도부터 안내가 이렇게 바뀌었다.

```
1~5회차:  "딜러 세션이 준비되지 않았습니다. 상점에 문의해주세요."   ← 맞는 안내
6회차~ :  "인증 시도가 너무 많습니다."                          ← 거짓
```

딜러는 자기가 OTP를 틀렸다고 믿고 재발급을 요청한다. 원인은 OTP가 아닌데
**사람을 엉뚱한 데로 보내는 안내다.**

게이트를 뒤로 옮기는 것은 답이 아니다(대조 한 라운드만큼 창이 열려 동시
버스트가 전부 통과한다 — T23이 50개로 재현한 그 실패다). **대조를 통과한
요청만 슬롯을 되돌린다.** 반납의 조건이 "OTP를 맞혔다"라서 추측하는 쪽은
닿지 못한다. 카운터가 막으려는 것은 모르는 사람의 반복 추측이지 아는 사람의
재시도가 아니다.

반납은 Lua 한 덩이다(`REFUND_SLOT`). 읽고-빼면 두 반납이 같은 값을 읽어
하나만 반영되거나 음수로 내려가는데, **음수는 잠금이 열리는 방향**이라
다음 추측 다섯 개가 공짜가 된다. 1 이하면 빼는 대신 키를 지운다 — 0으로
남기면 TTL만 있고 뜻이 없는 키가 된다.

### 동시 버스트 — 대기가 아니라 한도를 올렸다

부작용 1(한도보다 많은 태블릿이 동시에 인증하면 넘친 쪽이 OTP가 맞는데도
403)은 반납으로 안 닫힌다. 반납이 돌기 전에 INCR이 한도에 닿기 때문이다.

**대기를 검토했다** — "한도를 살짝 넘긴 요청은 bcrypt 한 라운드(~80ms)만큼
재웠다가 카운터가 내려갔으면 통과". 버린 이유 셋.

1. **거절이 공짜가 아니게 된다.** 지금은 INCR 한 번 하고 즉시 끝난다. 대기를
   넣으면 버릴 요청마다 서버가 100ms씩 핸들을 문다 — 이 게이트가 지키려던
   것(스레드풀·이벤트 루프)을 반대로 갉는다.
2. **불변식이 조건부가 된다.** "창당 정확히 `MAX_ATTEMPTS`개만 자격 검사에
   닿는다"는 회귀 테스트로 못 박혀 있는데(50개 버스트 → 정확히 그 수), 대기 뒤
   재검사를 넣으면 앞선 성공의 `clear` 여부에 따라 통과 수가 달라진다.
3. **읽고-검사가 되살아난다.** T23이 게이트를 INCR 자체로 만들며 지운 패턴이다.

**대신 `MAX_ATTEMPTS`를 5에서 10으로 올렸다.** 문제의 실체는 "동시 태블릿 수 >
한도"이고, 한도가 태블릿 수(= 테이블 수, 보통 5~10)보다 크면 상황 자체가
없어진다. 추측 방어는 안 변한다 — 10회 / 5분이면 시간당 120회고, 후보 10^6을
훑는 데 여전히 1년 가까이 걸린다. **잠금의 방어력은 창의 크기가 아니라
시도율에서 나온다.** bcrypt 부담도 창당 10회 × 80ms를 워커 넷이 나눠 지는
정도이고, 그 앞에 이번에 세운 분당 120 상한이 따로 있다.

스펙에 박혀 있던 `5`도 전부 `MAX_ATTEMPTS`로 바꿨다. 상수가 조정되는 값이 된
이상, 숫자를 박아 두면 다음에 고칠 때 스펙이 조용히 다른 것을 재게 된다.

### 잠금 정책은 그대로 뒀다

"5회 실패면 재발급해야만 풀린다"(자동 해제 없음)를 검토하고 버렸다.

- **얻는 것이 거의 없다.** OTP는 6자리 숫자라 후보가 100만이고 5회/5분이면
  시간당 60회다. 전부 훑는 데 1.9년 — 대회는 몇 시간이고 끝나면 OTP도 죽는다.
  **잠금의 방어력은 TTL이 아니라 시도율에서 나온다.**
- **잃는 것은 크다.** 영구 잠금은 틀린 OTP 다섯 개로 **상점 직원의 노동을
  강제**하는 것이다. 지금 문제를 완화하는 게 아니라 증폭한다.
- 6자리를 눈으로 읽어 태블릿에 넣는 일이라 오탐이 흔하다.

재발급이 즉시 탈출구인 것은 그대로다(`reissueDealerOtp`가 카운터를 지운다).
확인해 보니 **재발급은 `tokenVersion`을 올리지 않아 근무 중 딜러가 안 끊긴다** —
탈출구의 값이 싼 이유다. 전원 끊기는 `revokeDealerSession`이 따로 진다.

### RED 재현

| 검사 | expected / received |
|---|---|
| OTP가 맞으면 그 뒤에 막혀도 시도 초과로 바뀌지 않는다 | 메시지 한 종류 / 두 종류(6회차가 "시도가 너무 많습니다") |
| 맞은 OTP는 슬롯을 되돌린다 | `"남은 카운터 없음"` / `"남은 카운터 1"` |
| 틀린 OTP는 반납받지 못한다 | `UnauthorizedException` / `ForbiddenException`(반납이 추측까지 지우면 잠기지 않는다) |
| 한도를 넘긴 요청은 429로 끊긴다 | `"201,201,201 그다음 429"` / 데코레이터 없음 |
| `ThrottlerGuard`가 `APP_GUARD`로 등록돼 있다 | `"전역 가드 있음"` / `"전역 가드 없음"` |

마지막 검사를 따로 둔 이유: 429 스펙은 테스트 모듈이 가드를 직접 등록하므로
**제품이 등록했는지는 증명하지 못한다.** 빠뜨리면 데코레이터만 남고 아무도
세지 않는데 그 상태가 조용하다 — 요청이 전부 통과한다.

### 헤맨 자리 둘

**동적 `import()`가 던진다.** ts-jest가 CJS로 돌아
`A dynamic import callback was invoked without --experimental-vm-modules`가
났다. 데코레이터가 import 시점에 env를 읽으므로 값을 세운 뒤 들여야 하는데,
`await import()` 대신 `require`로 바꿨다.

**`jest.resetModules()` 뒤의 클래스는 다른 객체다.** 배선 스펙이 파일 맨 위에서
import한 `ThrottlerGuard`와 비교하다 "전역 가드 없음"을 받았다 — 배선은 맞는데
리셋 뒤 로드된 `@nestjs/throttler`가 별개 사본이라 identity가 어긋난 것이다.
같은 레지스트리에서 함께 꺼내도록 고쳤다.

### 기준선

백엔드 단위 198 → **208**, 통합 400 → **403**. contract 62 · 프론트 100 ·
e2e 13 · 타입 에러 0 그대로.

---

## T52 — 읽기 경로의 좌석 비트맵 재구성을 걷는다

**항목**: `backlog.md` B8의 이월 항목("비트맵 유실 복구가 `tables[0]`만 되살린다").
**범위**: `payment.service.ts`의 `getTournamentInfo` · 그 스펙 ·
`session.service.ts`의 `deleteTable` 주석과 그 스펙 주석.
**프론트 영향**: 없다. 응답 봉투(`{ tournament, seatStatus }`)가 그대로다.
**끝났다고 하는 기준**: 가드 없는 공개 조회가 Redis에 아무것도 쓰지 않는다.

### 문제 — 고칠 것이 아니라 지울 것이었다

이월 항목은 "`tables[0]`만 되살린다"로 적혀 있었다. 그대로 읽으면 할 일은
**다중 테이블로 넓히는 것**이다. 착수해서 코드를 보니 반대였다.

- **되살린 값이 응답에 반영되지 않는다.** `seatStatus`는 그 위에서 이미 읽은
  값이고, 재구성 뒤 다시 읽지 않는다(`let`으로 받아 놓고 재대입이 없었다).
  부르는 쪽이 얻는 것이 0인 **순수한 부수효과**였다.
- **읽기 경로가 쓰기다.** 게다가 `GET /tournaments/:id`는 가드가 없는 공개
  라우트다. 아무나 부르는 조회가 Redis 상태를 바꾼다.
- **권위가 둘이 된다.** 유실을 되세우는 일은 T46이
  `RecoveryService.recoverTournament`로 가져갔다 — 좌석 행이 있는 테이블
  **전부**를 스냅샷 기준으로 세운다. 여기서 0번만 세우면 남는 것은 유실도
  정상도 아닌 **세 번째 모양**(테이블 셋 중 1번만 있는 좌석 해시)이고, 다음에
  읽는 코드가 무엇을 믿을지 정할 수 없다.
- 조회 한 번에 `tournament.findUnique`가 두 번 나갔다. 두 번째는 오직 이
  재구성을 위한 것이었다.

그래서 넓히지 않고 걷었다. **T47(마감 판정 한 곳) · T48(되돌릴 수 없는 일은
마지막에) · T42(스냅샷 쓰기는 `mutateSnapshot` 하나)와 같은 자리다 — 권위를
하나로.**

### 실피해는 크지 않았다

발화 조건이 `seatStatus`가 비었고 `totalPlayers === 0`이다. 아무도 참가하지
않은 대회에서만 도는 경로라 좌석이 뒤집히는 종류의 손상은 없었다. **지운
근거는 손상이 아니라 경계다** — 공개 읽기가 쓰기를 하고, 복구 권위가 둘로
갈려 있었다.

### RED 재현

기존 스펙의 `'테이블이 있으면 예전처럼 비트맵을 되살린다'`를 뒤집고, 테이블
셋짜리를 하나 더 세웠다.

| 검사 | expected / received |
|---|---|
| 비트맵이 비어도 읽기 경로는 Redis에 쓰지 않는다 | `"남긴 필드 0개"` / `"남긴 필드 1개"` |
| 테이블 셋이어도 반쪽 상태를 만들지 않는다 | `"필드 0개 / 응답 좌석 0개"` / `"필드 1개 / 응답 좌석 0개"` |

두 번째의 `received`가 이 티켓의 그림 전부다 — **테이블이 셋인데 필드는 1개,
그런데 응답 좌석은 여전히 0개.** 반쪽만 되살렸고 그 반쪽조차 부르는 쪽에
닿지 않았다.

### 딸려 나온 것 — 죽은 근거 둘

`deleteTable`의 "마지막 테이블은 지울 수 없다"가 근거로 이 500을 들고 있었다
(제품 주석과 스펙 주석 두 곳). 그 500은 이미 `tables.length > 0` 가드로
사라졌고, 이제 경로 자체가 없다. **규칙은 남기고 근거만 갈았다** — 대회는
테이블이 최소 하나 있다는 전제 위에 서 있고(`createSession`이 1번을 함께
만든다), 0개가 되면 열려 있는데 아무도 앉을 수 없는 대회가 된다.

**규칙이 옳아도 근거가 죽으면 같이 고친다.** 죽은 근거를 남겨 두면 다음에
읽는 사람이 그 경로를 찾으러 가거나, 없는 것을 보고 규칙까지 지운다.

### 남은 TODO 주석 점검

같은 자리에서 제품 코드의 `TODO`를 전수로 봤다. 이 줄
(`// TODO : 다중 테이블 기능 개발시 유저자리 매핑하는로직`)이 **마지막
하나였고**, 지금 지웠다. `FIXME` · `HACK` · `@ts-ignore` · `@ts-expect-error`는
0건이다. 남은 린트 억제는 제품 코드 셋(`SeatGameClient` ·
`DealerGameClient` · `EliminatedOverlay`의 `react-hooks/exhaustive-deps`)과
e2e의 `no-console` 다섯이다.

### 남는 구멍 — 주인이 없지, 자리가 없는 게 아니다

재기동 없이 좌석 해시만 날아가는 유실은 여전히 아무도 고치지 않는다. 옛
코드도 그걸 고치지 못했다(사람이 없는 대회의 0번 테이블만, 응답 밖에서).
**T52는 그 구멍을 넓히지도 좁히지도 않았다** — 막는 척하던 코드를 치웠을 뿐이다.

지금 손대지 않는 이유는 **부하·단위·통합 어느 테스트도 이것을 아직 발견하지
않았기 때문**이다. 발견되면 그때 고친다. 이 리포에서 "실피해가 없다"는 미루는
근거로 이미 두 번 쓰였고(T42를 하네스 뒤로, T25의 마이그레이션 사전 점검),
둘 다 **재현 경로가 생기면 앞으로 당긴다**는 조건이 붙어 있었다. 여기도 같다.

**그리고 이 구멍의 주인은 상점 콘솔이 아니다.** 초안에는 "복구를 상점 콘솔
버튼으로 노출하는 쪽"이라고 적었는데 틀렸다 — 상점은 이 SaaS의 **손님**이다
(`domain.md`의 「상점도 손님이다」). 손님 화면에 인프라 복구 버튼을 두면 장애가
손님 책임이 된다. B2가 운영자 조작 화면을 안 만든 근거도 같은 자리였다
("재구성이 자동이라 상점이 판단할 것이 남지 않는다").

착석 경로에서 자가치유하는 길도 **일부러** 닫혀 있다 — `UPDATE_SEAT_BIT`은
필드가 없으면 아무것도 하지 않는다(T48). "없으면 만든다"를 쓰기 경로에 두면
삭제와 유실이 같은 모양이 되어, 지워진 테이블이 좌석 목록에 되살아나던 그
버그가 돌아온다. 남는 자리는 자동 감지(부팅 복구의 확장)뿐이고, 그것을 언제
어디에 걸지는 플랫폼 운영 도구가 생기는 날 정해진다(B10과 같은 갈래).

### e2e의 찬 시동 하나

첫 e2e 실행에서 `console.spec.ts`의 첫 스펙만 빨갰다 —
`main: 대회를 찾을 수 없습니다.`. 이 화면의 서버 컴포넌트는
`GET /tournaments/:id`가 실패하면 그 문구를 그리므로, 이 티켓이 만진 바로 그
엔드포인트를 의심할 자리다. **같은 스펙을 혼자 다시 돌려 통과했고, 시드 뒤
전체를 다시 돌려 13건 전부 통과했다.** 스펙 파일 알파벳 순서상 이것이 가장
먼저 도는 스펙이라, 백엔드 부팅과 Next의 첫 라우트 컴파일이 겹치는 자리에
5초 단언이 걸린 것이다. 제품 변경과 무관하다.

### 기준선

통합 399 → **400**(29 suites 그대로 — 1건 대체 + 2건 추가). contract 62 ·
백엔드 단위 198 · 프론트 100 · e2e 13 · 타입 에러 0 그대로.
...
**범위**: `session.service.ts`의 `updateSession` · `session.controller.ts` ·
그 스펙들.
**프론트 영향**: 없다. `PATCH /store/sessions/:id`에 호출부가 아직 없다
(같은 이월 목록의 별도 항목).
**끝났다고 하는 기준**: 남의 대회를 수정하면 403이고, 값이 하나도 바뀌지 않는다.

### 문제

운영 조작에 소유권 검사를 넣는 일은 T23(재발급·내보내기)에서 시작해
T25(테이블 추가·삭제)·T49(취소)까지 왔는데 **수정 경로만 그대로였다.**
`assertTournamentOwnership`의 주석이 "그건 이 태스크 이전부터 있던 별도
항목이라 여기서 같이 고치지 않는다"고 스스로 가리키고 있었다.

뚫린 것이 작지 않다. **참가비 · 시작 스택 · 블라인드 구조 · 상금 분배율**이
전부 이 한 경로로 바뀐다 — 상점 관리자 역할만 있으면 남의 대회 참가비를 0으로
만들거나 분배율을 자기 쪽으로 몰 수 있었다.

### 딸려 있던 500

같은 메서드가 없는 대회에 **500**을 냈다. `getGameSession`이 준 `null`을
`if (session && ...)`가 그냥 지나가서, 뒤의 `tournament.update`가 P2025로
죽었다 — 없는 것을 물었을 뿐인데 서버 오류다. `revokeDealerSession`이 같은
모양이었고 T23이 고쳤다. 소유권 검사 한 줄이 이것도 함께 닫는다(그 함수가
`!tournament`면 404를 던진다).

### RED 재현

| 검사 | expected / received |
|---|---|
| 다른 상점 소유자가 수정을 호출하면 거부한다 | `rejects` / **`resolved`** |
| 거부된 수정은 값을 하나도 바꾸지 않는다 | `rejects` / **`resolved`** |
| 없는 대회를 수정하면 404다 | `NotFoundException` / `PrismaClientKnownRequestError` |

"거부한다"와 "값이 안 바뀐다"를 **따로 둔** 이유는, 검사를 첫 문장에 두는 것의
요점이 던지기 **전에** 아무것도 안 하는 것이기 때문이다. 타입만 보면 나중에
검사를 뒤로 옮겨도 초록이다.

### 시그니처를 바꾼 것이 검증을 대신했다

`updateSession(id, dto)` → `updateSession(id, dto, ownerId)`로 인자를 늘리자
**컴파일러가 호출부 셋을 전부 잡았다**(컨트롤러 하나 · 스펙 둘). 검사를
옵셔널 인자로 붙였으면 빠뜨린 호출부가 조용히 남았을 자리다.

그중 단위 스펙 하나는 `tournament.findUnique` 목이 `store`를 들고 있지 않아
소유권 확인에서 죽었다. 목을 `store: { ownerId }`까지 세워, **본인 소유인데도
닫혀서 거부되는 것**을 보도록 고쳤다 — 아니면 그 409 테스트가 403으로 통과해
검증 대상에 닿지 못한다.

### 기준선

통합 390 → **394**(28 suites). contract 62 · 백엔드 단위 198 · 프론트 100 ·
e2e 13 · 타입 에러 0 그대로. e2e는 돌리지 않았다 — 프론트 코드가 바뀌지 않았고
이 엔드포인트에 호출부가 없다.
