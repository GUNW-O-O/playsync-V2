...
동시 리바인·탈락은 넣지 않았다 — 판정자가 세 번째와 같은 테이블 락이라 새로
증명하는 것이 없다. 램프 실행(T40, 재번호 후 T41)에서 실제 부하로 지나간다.

### 결정

**앞 둘(유니크 제약)은 예상대로 한 번에 GREEN이자 증거였다.** `Promise.allSettled`로
동시에 던지면 성공 하나, 실패 나머지가 정확히 나오고, 유니크 제약을 주석 처리해
빨간불을 확인한 뒤 되돌렸다(Task 1·2 — 이 절의 나머지 밖).
...
|---|---|---|---|
| Task 1 (좌석 유니크) | `@@unique([tableId, seatPosition])` 주석 처리 | 3회 FAIL | 성공 5(원래 1) — 다섯이 전부 앉음 |
| Task 2 (대회·유저 유니크) | `@@unique([tournamentId, userId])` 주석 처리 | 3회 FAIL | 성공 2(원래 1) — 같은 사람이 두 테이블에 |
