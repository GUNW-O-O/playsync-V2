...
**닫히는 것은 한 방향뿐이다.** 해제 → 입장은 막힌다(입장의 비트 쓰기가 항상
마지막에 올바른 값으로 도착한다). 반대 방향 — 입장의 비트 1이 자기 락 **밖**에
있어서 뒤이은 해제의 비트 0보다 늦게 도착하는 경우 — 는 그대로 남는다. T28이
입장의 레디스 쓰기를 락 밖에 둔 데서 오는 것이고 T29 이전부터 있었다.
...
뗐다고 믿는데 한 명만 떨어진다. 검사는 같은 `FOR UPDATE`로 직렬화되도록
트랜잭션 안, 검사 2 옆에 둔다. **이것이 T30을 고치지는 않는다** — 해제가 그
버그에 먹이를 주는 것만 끊는다.
