...
| 12,000명 재실측 | T76이 측정 결함을 걷어냈지만, **그것이 1,061ms의 전부였는지는 재 봐야 안다.** `docs/results/`에 실행 하나를 더 남기는 일이고 코드 변경이 아니다. 대장은 Docker VM 15.57GB를 요구하는데 **이 기계의 VM이 16.72GB다 — 제약은 이미 풀렸다.** 남은 것은 돌리는 일뿐이다 | 대기 |
