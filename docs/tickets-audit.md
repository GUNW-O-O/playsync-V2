...
> 것**이 여기 있고, 착수하면 [`tickets-next.md`](./tickets-next.md)에 티켓으로
> 옮겨 적는다. 두 곳에 같은 내용을 두지 않기 위해, 옮긴 항목은 여기서 상태만
> 바꾸고 서술은 그쪽에 쓴다.
>
...
| # | 티켓 | 무엇이 깨지나 | 등급 | 상태 |
|---|---|---|---|---|
| T58 | 앤티가 사이드팟에 담기지 않는다 | 칩이 증발한다 | 치명 | 대기 |
| T59 | 동시 파산의 등수와 상금 | 상금이 두 번 나가고 대회가 안 닫힌다 | 치명 | 대기 |
| T60 | 인원수가 DB와 Redis에서 갈라진다 | 최후 1인 판정과 등수가 통째로 틀어진다 | 치명 | 대기 |
| T61 | 시작 준비가 락 없이 스냅샷을 덮어쓴다 | 방금 앉은 사람이 게임에서 사라진다 | 치명 | 대기 |
| T62 | 체크포인트 재시도가 던지면 테이블이 갇힌다 | 어떤 조작으로도 다음 핸드로 못 간다 | 높음 | 대기 |
| T63 | 휴식 레벨이 등록을 영구히 닫는다 | 리바인이 예정보다 일찍 죽는다 | 높음 | 대기 |
| T64 | 대회 입력이 검증을 지나가지 않는다 | 포인트가 찍히고 전광판이 멎는다 | 높음 | 대기 |
| T65 | 비턴 액션이 현재 턴의 타이머를 리셋한다 | 제한시간을 무한히 늘릴 수 있다 | 높음 | 대기 |
| T66 | 가드 없는 읽기 경로 | 남의 대회·상점이 그대로 나간다 | 높음 | 대기 |
| T67 | 좌석 태블릿이 실패를 삼킨다 | 참가자가 모르는 채로 탈락한다 | 높음 | 대기 |
| T68 | 레이즈 입력이 낼 수 있는 금액과 어긋난다 | 합법적인 레이즈 불가 / 조용한 올인 | 중간 | 대기 |
| T69 | 체크포인트 실패의 탈출구가 화면에 없다 | 멈춘 테이블에서 나올 길이 없다 | 중간 | 대기 |
| T70 | 콘솔 좌석 선택이 새로 그린 판을 따라간다 | 엉뚱한 사람이 좌석에서 빠진다 | 중간 | 대기 |
| T71 | 계약·스키마의 드리프트 | 아직 증상 없음. 다음 필드에서 터진다 | 낮음 | 대기 |
