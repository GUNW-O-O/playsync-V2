...
| T93 | 소켓이 끊기면 단말이 스스로 돌아오지 않는다 | 서버를 재시작하면 태블릿 전부가 에러 문구에서 멈춘다. 사람이 한 대씩 새로고침해야 한다 | 높음 | — | **완료 (#114)** |
| T94 | 재시작이 차례였던 사람을 자동 폴드시킨다 | 정지 동안 흐른 턴 타이머가 부팅 직후 발화한다. 돌아온 사람이 누른 버튼도 시간 초과로 바뀐다 | 치명 | — | **완료 (#113)** |
| T95 | 정지와 재개가 화면에 없다 | 서버가 멈췄다 돌아온 것을 아무 단말도 말하지 않는다 | 높음 | T93 · T94 | **완료 (#115)** |
| T96 | 소켓이 없는 화면은 정지를 모른다 | 참가자 폰과 전광판이 멈춘 대회를 정상 진행처럼 보여준다 | 중간 | T95 | **완료 (#116 · #117)** |
...
**재실측 없이 닫았다(2026-09-12).** 원인(지각이 연 고아 창에 다음 사람의
브로드캐스트가 붙어 생각 시간이 지연으로 기록됐다)과 수정(지각은 창을 열지
않는다 · `serverTime` 도장 가드)이 코드와 실험 양쪽에서 확정됐다. 남은 질문
「그것이 1,061ms의 전부였나」는 12,000명 무대를 다시 세워야만 답이 나오는데,
그 답이 바꿀 결정이 없다 — 1코어 배포 판단은 이미 `my_action_server_ms`·
`my_action_client_ms` 분해가 다음 실측에서 가른다.
...
거기가 `TournamentStatus.SYNCING`이 앉을 자리다. T96이 그 값을 되살렸다 — 이
절을 쓸 때는 "선언만 된 값이라 마이그레이션이 필요 없다"고 적었지만, **T71이
이미 지운 뒤였다**(`20260821120000_drop_unused_schema_declarations`). 그래서
마이그레이션이 필요했다.

**함정은 T71이 먼저 닫았다.** `getStoreAvailableSessions`와
`getGameSessionWithTables`는 `in: [ONGOING, PENDING]`이 아니라 여집합
(`NOT_CLOSED_TOURNAMENT_FILTER`)으로 거른다. 대신 T96이 `status === ONGOING`
직접 비교 넷을 찾아 `LIVE_TOURNAMENT_STATUSES`로 모았다 — 그중 하나가 부팅
복구 조회라, 복구 중에 다시 죽은 대회가 복구 대상에서 빠질 뻔했다.
...
재개 순서를 **좌석 먼저, 딜러 나중**으로 고정한다. 딜러 단말이 자기 재접속을
무리가 흩어지는 폭만큼 늦춘다.

**얻는 것이 겉보기와 다르다.** "딜러가 일찍 못 누르게 막는다"가 아니라
**딜러가 보는 그림이 이미 가라앉은 뒤라는 것**이다. 먼저 붙으면 아홉 중 둘만
찬 테이블을 보게 되고, 사람이 버튼을 이르게 누르는 순간이 정확히 거기다.
나중에 붙으면 아홉이 다 찼거나, **어느 둘이 비었는지**를 본다.

**그러려면 딜러 화면이 누가 돌아왔는지를 보여야 한다.** 지금은 아무것도 안
보여준다 — 스냅샷의 좌석에 접속 여부가 없고, `WsGateway.handleDisconnect`는
브로드캐스트 집합에서 소켓을 빼는 것이 전부다. 순서만 바꾸고 화면이 그대로면
딜러는 여전히 아홉 좌석을 다 본다.

**여기서는 좀비 소켓이 허용된다.** 반만 닫힌 TCP는 `onclose`가 안 떠서
"붙어 있음"이 거짓일 수 있는데(게이트웨이에 하트비트가 없다), 이 값을
**판정이 아니라 표시**로 쓰면 문제가 안 된다 — **딜러에게는 눈이 있다.**
좌석에 사람이 없으면 화면이 뭐라 하든 딜러가 안다. 같은 값을 타이머를 멈추는
게이트로 쓰면 좀비 하나가 테이블을 영영 묶는다. 쓰는 자리가 성질을 정한다.

**늦추는 폭은 실측에 건다.** 감으로 10초를 주면 무리가 31.9초에 흩어지는 날
순서가 그대로 깨진다. 좌석 재접속에 쓰는 것과 **같은 상수**를 쓴다.

**이것은 창을 줄이지 닫지는 않는다.** 딜러가 나중이어도 태블릿이 꺼진
참가자는 여전히 없다. 닫는 것은 딜러의 판단이고, 이 순서는 그 판단이
**흔들리는 그림 위에서 내려지지 않게** 하는 장치다.

### 같은 결함의 더 큰 판

**재시작과 무관하게, 딜러는 지금 언제든 깜깜한 좌석을 게임에 넣을 수 있다.**
막는 것이 없고, 그 사람은 30초 뒤에 자동 폴드된다. 재시작은 그 일이 **전
테이블에서 동시에** 일어나는 경우일 뿐이다. 일반 경우는 범위가 훨씬 넓어
따로 세운다 — 여기서는 재시작 경로만 닫는다.
...
**등급**: 중간 · **범위**: `backend/src/recovery` · `backend/src/ws` · `backend/prisma` · `packages/contract` · `frontend` · **의존**: T95

T95가 정지를 테이블 스냅샷(`resumePending`)에 올렸지만, REST로 읽는 참가자 폰과
전광판은 그것을 못 봤다. 그리고 블라인드 시계는 **부팅 순간부터** 흘러, 태블릿들이
돌아오는 동안의 시간이 게임 시간으로 샜다 — 부팅에서 다운타임만큼 일괄로 미는
방식은 대회마다 다른 복귀 시각을 무시했다.

설계와 기각한 안은 [스펙](./superpowers/specs/2026-09-11-t96-syncing-design.md),
지금 코드의 규칙은 `domain.md`의 「서버가 죽어도 대회는 계속된다」.

### 무엇으로 닫았나

- **#116** — 좀비 소켓. 게이트웨이가 ping/pong으로 반만 닫힌 연결을 치우고,
  태블릿은 `keepalive` 침묵으로 자기가 끊긴 것을 안다. **소켓 수를 판정에 쓰려면
  그 수가 사실이어야 해서** 먼저 들어갔다.
- **#117** — `SYNCING`. 부팅이 켜고 그 대회의 딜러가 n/n으로 돌아오면 끈다. 그
  사이 블라인드가 `pausedAt`에서 멈추고, 끄는 자리에서 한 번 민다.

### 기존 테스트가 왜 못 잡나

부팅 보정을 단언하던 복구 검사 일곱이 **보정이 부팅에 있다는 것 자체**를 사실로
삼고 있었다. 틀린 시점을 정확히 검증하고 있었으니 초록이었다. T96이 그 일곱의
의도를 유지한 채 확인 시점을 `completeSync` 뒤로 옮겼다.
...
**열린 줄이 넷이다**(jest 워커 죽음 · 라이브러리의 타이머 전역 삭제 · 동시
시작의 좁은 창 · 좀비 청소의 틱 비용). T76의 재실측은 재지 않고 닫았다(T76 절
끝). 나머지는 지운 것이 아니라 남겨 둔다 —
...
| 12,000명 재실측 | T76이 측정 결함을 걷어냈지만 그것이 1,061ms의 전부였는지는 재 봐야 안다고 남겨 뒀다. **재지 않고 닫는다** — 원인과 수정이 코드와 실험으로 확정됐고, 답이 바꿀 결정이 없다(T76 절 끝) | 닫음 (재실측 안 함) |
...
| `WsGateway.sweepSockets`의 틱 비용 | 10초마다 모든 소켓에 `ping()`과 `keepalive`를 동기로 보낸다. 12,000명 무대면 틱마다 쓰기 약 2.4만 번이라 `my_action_server_ms`에 10초 주기 스파이크로 보일 수 있다. 다음 램프에서 본다(T96 PR 1) | 대기 (측정) |
| `WsGateway.recount`의 k 표시 | `broadcast`가 OPEN이 아닌 소켓을 방에서 먼저 지우면 뒤이은 `handleDisconnect`의 `Set.delete`가 false라 재집계가 안 돈다. n/n 판정은 OPEN 딜러만 세어 안전하고, 다른 딜러의 「k/n 복귀」 표시만 다음 이벤트까지 높게 남는다. 감수했다 | 감수 (T96) |
