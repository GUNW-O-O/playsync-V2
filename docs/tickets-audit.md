...
| # | 티켓 | 무엇이 깨지나 | 등급 | 파 | 상태 |
|---|---|---|---|---|---|
| T58 | 앤티가 사이드팟에 담기지 않는다 | 칩이 증발한다 | 치명 | 2 | 대기 |
| T59 | 동시 파산의 등수와 상금 | 상금이 두 번 나가고 대회가 안 닫힌다 | 치명 | 3 | 대기 |
| T60 | 인원수가 DB와 Redis에서 갈라진다 | 최후 1인 판정과 등수가 통째로 틀어진다 | 치명 | 2 | 대기 |
| T61 | 시작 준비가 락 없이 스냅샷을 덮어쓴다 | 방금 앉은 사람이 게임에서 사라진다 | 치명 | 3 | 대기 |
| T62 | 체크포인트 재시도가 던지면 테이블이 갇힌다 | 어떤 조작으로도 다음 핸드로 못 간다 | 높음 | 4 | 대기 |
| T63 | 휴식 레벨이 등록을 영구히 닫는다 | 리바인이 예정보다 일찍 죽는다 | 높음 | 4 | 대기 |
| T64 | 대회 입력이 검증을 지나가지 않는다 | 포인트가 찍히고 전광판이 멎는다 | 높음 | **1** | 완료 (#60) |
| T65 | 비턴 액션이 현재 턴의 타이머를 리셋한다 | 제한시간을 무한히 늘릴 수 있다 | 높음 | 5 | 대기 |
| T66 | 가드 없는 읽기 경로 | 남의 대회·상점이 그대로 나간다 | 높음 | 2 | 대기 |
| T67 | 좌석 태블릿이 실패를 삼킨다 | 참가자가 모르는 채로 탈락한다 | 높음 | 3 | 대기 |
| T68 | 레이즈 입력이 낼 수 있는 금액과 어긋난다 | 합법적인 레이즈 불가 / 조용한 올인 | 중간 | 3 | 대기 |
| T69 | 체크포인트 실패의 탈출구가 화면에 없다 | 멈춘 테이블에서 나올 길이 없다 | 중간 | 5 | 대기 |
| T70 | 콘솔 좌석 선택이 새로 그린 판을 따라간다 | 엉뚱한 사람이 좌석에서 빠진다 | 중간 | 2 | 대기 |
| T71 | 계약·스키마의 드리프트 | 아직 증상 없음. 다음 필드에서 터진다 | 낮음 | 4 | 대기 |

아래 **잔여 목록**은 티켓을 따로 세우지 않은 것들이다. 가까운 티켓에 묻어 간다.

## 작업 순서와 병렬 묶음

등급만으로 순서를 정하면 같은 파일에서 부딪힌다. **같은 파에 있는 것은 만지는
파일이 서로 겹치지 않으므로 동시에 돌릴 수 있고**, 파가 넘어가는 자리는 파일이
겹치거나 앞의 결정에 기대는 자리다.

파일이 아니라 **함수**로 갈랐다. `redis.service.ts`는 넷이 만지지만 T60은
`eliminatedPlayer`·`seatPlayer`·`joinPlayer`, T61은 `saveInitialTableSnapshots`·
`mutateSnapshot` 계열, T63은 `checkAndSyncBlindLevel`, T64는
`recalculateAvgStack`이라 서로 안 닿는다. 파일 이름만 보고 직렬화하면 넷을
한 줄로 세우게 된다.

### 착수 전에 사람이 정해야 하는 것 셋

코드가 아니라 규칙이 없는 자리다. 정하지 않으면 그 파가 멈춘다.

| 무엇 | 어디 | 언제까지 |
|---|---|---|
| `entryFee: 0`을 허용할 것인가 | T64 6-3 | **1파 전.** 막으면 `seed-load.ts`와 부하 무대가 함께 움직인다 |
| 앤티 금액의 정수 보장을 무엇으로 할 것인가 | T58 | 2파 전. 구조가 앤티를 직접 들지, `sb`에 배수 제약을 걸지 |
| 동시 파산의 등수를 무엇으로 가를 것인가 | T59 | 3파 전. 핸드 시작 스택이라면 `resolveWinners`가 그 값을 들고 나와야 한다 |
| 휴식을 별도 필드로 옮길 것인가 | T63 | 4파 전. 계약 변경이면 프론트·시드·복구가 함께 움직인다 |

### 1파 — T64 혼자 (병렬 없음)

**혼자인 이유는 넓어서다.** `session.controller` · `session.service` ·
`shared/dto/` · `shared/util/util.ts` · `redis`의 `recalculateAvgStack` ·
`schema.prisma`(유니크 스코프 마이그레이션)를 건드려서 뒤의 거의 모든 파와
닿는다. 먼저 지나가면 나머지가 자유롭다.

그리고 **셋이 이 결정 위에 선다.**

- T58의 앤티 정수 보장 — `BlindLevelDto`가 지금 **한 번도 실행되지 않는다**(6-1).
  입구가 안 막힌 채로 엔진만 고치면 소수 스택이 그대로 들어온다
- T63의 선택지 3(블라인드 구조 검증)이 같은 DTO 자리다
- T71 9-3의 `Int` 범위와 6-6의 유니크가 같은 마이그레이션이다

묻어갈 잔여: `CreateTournamentDto`의 `@Max` 없음(postgres `integer` 초과 시 22003).

### 2파 — 넷 동시

| 티켓 | 만지는 곳 | 겹침 |
|---|---|---|
| T58 앤티 | `table-engine.ts`의 `payAnte`·`executeBet`, `src/scenario/` | 없음 |
| T60 인원수 | `dealer.service`·`entry.service`·`redis` 카운터 셋·`prisma/seed.ts` | 없음 |
| T66 가드 | `playsync`·`payment`·`dealer` 컨트롤러, 콘솔 `page.tsx` | 없음 |
| T70 콘솔 좌석 | `ConsoleClient.tsx` | 없음 |

T66과 T70이 같은 콘솔 디렉터리지만 **T66은 서버 컴포넌트(`page.tsx`)와 미들웨어,
T70은 클라이언트 컴포넌트**라 파일이 다르다. 다만 인접하므로 한 사람이 잡으면
왕복이 준다.

묻어갈 잔여: T60에 `recalculateAvgStack`의 `parseInt(active || '1')`,
T66에 `EntryController`·`PaymentController`의 같은 베이스 경로와
`middleware.ts`의 `config.matcher`.

### 3파 — 넷 동시

| 티켓 | 만지는 곳 | 무엇을 기다렸나 |
|---|---|---|
| T59 동시 파산 | `playsync.service`의 `eliminatePlayer` | T60 — 카운터가 맞아야 등수가 의미를 갖는다 |
| T61 락 없는 스냅샷 | `session.service`의 `initializeGame`, `redis` 스냅샷 계열 | T64 — 같은 `session.service` |
| T67 좌석 태블릿 | `(terminal)/table/` 셋 | 없음 |
| T68 레이즈 입력 | `SeatActionPanel.tsx`, `table-engine`의 `handleRaise` | T58 — 같은 엔진 파일 |

묻어갈 잔여: T67에 `WaitingClient.submit`의 `OTP_LENGTH` 미대조 · 폴링의
`try`/`catch` 없음 · `auth/action.ts`의 `res.ok` 전 `json()`, T68에
`ActionTimer`의 서버 시각 보정 없음.

### 4파 — 셋 동시

| 티켓 | 만지는 곳 | 무엇을 기다렸나 |
|---|---|---|
| T62 retry 막다른 곳 | `common/retry.ts`, `playsync.service`의 체크포인트 | T59 — 같은 `playsync.service` |
| T63 휴식 레벨 | `registration.ts`, `redis`의 `checkAndSyncBlindLevel` | T64 — 구조 검증안을 고른 경우 |
| T71 계약 드리프트 | `packages/contract`, `frontend/types/game.ts`, `ws.gateway`, `schema.prisma` | T64 — 같은 마이그레이션 |

묻어갈 잔여: T71에 `WsIdentity.role`이 `SEAT_ROLE = 'PLAYER'`를 싣는 것.

### 5파 — 둘 동시

| 티켓 | 만지는 곳 | 무엇을 기다렸나 |
|---|---|---|
| T65 비턴 타이머 | `playsync.service`의 `handleAction`, `ws.gateway` | T62(같은 파일) · T71(같은 게이트웨이) |
| T69 체크포인트 탈출구 | `DealerGameClient.tsx`, `frontend/types/game.ts` | T62(탈출구가 실제로 열려 있어야 한다) · T71(타입) |

**T71을 먼저 두는 이유가 여기 있다.** `contract`의 `TableStateSchema`에
`dbSyncStatus`가 **이미 있다.** T71 9-2로 프론트가 손 복사본을 버리고 contract를
import하면 T69의 타입 구멍이 같이 닫힌다 — T69에서 `types/game.ts`에 필드를
손으로 더하면 **드리프트를 한 번 더 만드는 것**이다.
...
2파   T58            T60            T66            T70
       │              │              │
3파   T68            T59            T67   T61(1파에서 이어짐)
       │              │
4파    │             T62            T63(1파)   T71(1파)
       │              │                          │
5파    └──────────── T65 ───────────────────────┘
                    T69 ←── T62 · T71
```

최대 동시 넷. 파가 다섯이라 **직렬로 열넷을 도는 것보다 아홉 자리 짧다.**

---

## T58 — 앤티가 사이드팟에 담기지 않는다

**등급**: 치명 (칩 보존 위반) · **범위**: `game-engine/table-engine.ts`, `src/scenario/` · **프론트 영향**: 없음
...
- 앤티 금액의 정수 보장을 **규칙으로** 정한다. 코드가 임의로 `Math.floor`하지
  말고, 블라인드 구조가 앤티 금액을 직접 들지 아니면 `sb`에 배수 제약을 걸지를
  먼저 결정한다.
...
| `TransactionType.CHARGE` | 사용처 0건. 포인트 충전 경로가 없다 — `seed-load.ts`가 `entryFee: 0`을 쓰는 이유가 이것이다(T64와 얽힌다) |
