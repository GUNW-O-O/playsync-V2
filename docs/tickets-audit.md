# Playsync V2 전수검사 결함 대장 (T58~T92)

> **T58~T71은 코드만 읽어서 만든 목록이다.** 2026-08-19에 `backend/src` · `frontend/src` ·
> `packages/contract` · `backend/prisma`를 문서 없이 전수로 훑고, 그중
> **재현했거나 코드 경로를 끝까지 추적한 것**만 남겼다. **T72~T92는 그것을
> 고치고·찍고·재다 나온 후속이다** — 출처는 각 절 첫머리에 적었다.
>
> **장애 복구 경로(T93~)는 [`tickets-recovery.md`](./tickets-recovery.md)로
> 갈라 냈다.** 새 결함이 이 목록의 후속이 아니면 여기 붙이지 않는다.
>
...
T93~T96은 [`tickets-recovery.md`](./tickets-recovery.md)에 있다.
...
[`tickets-recovery.md`](./tickets-recovery.md)로 갈라 냈다.
...
**열린 줄이 셋이다**(jest 워커 죽음 · 라이브러리의 타이머 전역 삭제 · 동시
시작의 좁은 창). 좀비 청소의 틱 비용은 `tickets-recovery.md`로 옮겼다. T76의 재실측은 재지 않고 닫았다(T76 절
