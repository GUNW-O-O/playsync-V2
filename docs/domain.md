...
돌려주면, 참가자는 **돈이 빠진 채 실패 화면**을 보고 다시 눌러 409를 받는다 —
그 미러는 영영 안 써진다. 세 자리가 그 모양이었다(`joinSession` ·
`enterSeat` · `finishClose`). 던지지 않고, `down`이면 시도조차 하지 않고
(재시도 예산 7~10초를 물지 않으려고) 복구 뒤에 한 번 돈다.
