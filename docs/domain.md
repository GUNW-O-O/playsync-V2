...
(먼저 지워야 「스냅샷 없음」으로 끝난다). **미뤄 둔 정리는 `whenUp` 등록 순서상
리바인 고리보다 늦게 깨어난다** — 그 구간에서는 스냅샷이 살아 있으므로
「스냅샷 없음 == 닫혔다」가 성립하지 않고, `DealerService.closedTables`가 그
몫을 든다. 정리 실패로 요청을 실패시키지는 않는다 — 닫힘은 이미 커밋됐다.
