...
아니다. 프론트가 210에서 215로 는 것은 T87·T88이 닫힌 대회의 표시를 갈라
적게 되면서고, 백엔드가 374에서 375로 는 것은 **계약과 백엔드가 닫힘 목록을
두 벌로 든다**는 사실에 검사를 하나 건 것이다 — 다섯 번째 상태가 한쪽에만
들어가면 두 화면이 조용히 T87·T88로 되돌아간다.
...
**T87·T88은 통합도 다시 돌리지 않았다** — 백엔드 런타임 코드가 바뀌지 않았다.
늘어난 것은 단위 스펙 하나(닫힘 목록 대조)이고, 나머지는 전부 프론트 표시다.
