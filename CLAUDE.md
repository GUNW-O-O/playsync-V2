...
**사람 기록물 — 작업 중에 읽지 않는다.**
`README.md` · `docs/archive/`(1단계 대장 · MVP 리뷰) · `docs/results/`.

같은 내용을 두 곳에 쓰지 않는다. 두 벌이 되면 어긋난다.

**문서 하나는 주제 하나를 든다. 가볍게, 핵심만.** 성격이 다른 것을 있는 문서에
계속 붙여 키우지 않고 새 문서를 연다. **완료된 티켓은 대장에 표 한 줄과 PR 번호만
남긴다** — 본문은 PR과 커밋에 있다. 대장 셋이 완료 본문을 들고 4,000줄을 넘긴 뒤
(2026-09-19) 그렇게 정리했다.
...
**PR마다 그 브랜치에서 실제로 돌린 값을 적는다** — 여러 PR이 각자 잰 숫자를
합산하지 않는다. **줄마다 이번에 잰 값인지 본다.** 안 잰 줄은 조용히 낡는다 — e2e
13이 T84부터 마무리 PR까지 깨진 채 기준선에 남아 있었다.

e2e · 촬영 · 부하 하네스 · 실제 kill은 CI가 아니라 사람이 돌린다.

- **정산 촬영은 마무리 셋(`DEMO_ENDING`)을 각각 시드부터 돈다** — 하나가 대회를
  닫으면 나머지는 찍을 자리가 없다. 하나가 7분 안팎, 셋 합쳐 21분이다.
- **리바인 수락은 서버 창(15초) 안에서 눌러야 한다.** `RebuyOverlay`는 마감이
  지나도 화면에 남아 `toBeVisible`이 죽은 프롬프트에도 통과한다.
- **상점에 대회가 여럿이면 대기 화면의 기본 선택은 최신 대회다.** 시드가 무대
  둘을 같은 상점에 깔므로 e2e와 촬영은 대회를 먼저 고른다(`pickTournament`).
- **슬레이트 시각은 페이지 안에서, 칠해진 뒤에 읽는다.** 노드의 `Date.now()`는
  면마다 다른 페인트 지연을 오차로 남긴다.

`tsc`가 이미 지운 파일의 에러를 계속 보고하면 `.tsbuildinfo`가 낡은 것이다.
`backend/dist`를 지우고 다시 돌린다.
...
**새 테스트가 처음부터 통과하면 의심한다. 실패를 먼저 본다.** 사후에 추가한
검사는 제품 코드를 일부러 되돌려 빨간불을 확인한다(`git stash push <파일>` 또는
임시 편집 후 복원). 되돌려 본 빨간불이 초록이면 **고친 것이 아니라 검사를 의심한다.**

데인 모양들:

- **검사가 닿지 못했다** — 다른 계층이 이미 막고 있었거나, 기존 상수와 결과가
  같은 입력을 골랐다
- **검사가 둘이면 둘이 어긋나는 입력이 있어야 한다** — 일치하는 입력만 먹이면
  하나를 통째로 지워도 초록이다(T29). 같은 이유로 **반대 입력**이 없으면 「언제나
  X」 구현이 나머지를 전부 통과한다
- **순서는 타이밍이 아니라 조회를 조종해서 강제한다** — 목이나 스파이로 한쪽을
  붙잡고 다른 쪽을 먼저 흘린다. 실제 왕복 타이밍에 맡긴 경합 회귀는 고치기 전
  코드에서도 통과했다
- **값만 보는 단언은 「안 돌았다」와 「돌았는데 값이 같다」를 못 가른다** — 일이
  **한 번 더 일어났다**를 세야 빨간불이 선다
- **되돌림 하나가 초록이라고 그 코드가 죽은 것이 아니다** — 막는 것이 둘 겹쳐
  있을 수 있다. 둘 다 빼 보고 판단한다
- **우회한 단언은 결함의 주소다** — 어떤 키를 직접 못 보고 돌아서 확인했다면 그
  자리를 본다
- **되돌림이 어디서 멎는지가 증상을 말해 준다** — 예상한 단계가 아니면 설명이 틀렸다
- **증상을 확인하러 가지 말고 그 자리에서 실제로 무슨 일이 일어나는지 본다** —
  티켓의 서술이 틀린 채로 결함의 모양인 경우가 있었다(T108)
- **파일 단독 초록은 부하 없는 초록이다** — 전체 실행에서만 빨개지는 가짜 타이머
  검사가 있었다
- **읽은 값이 제정신인지 먼저 보는 단계를 둔다** — 그래야 계산이 아니라 읽기가
  빨개져 원인이 읽힌다(`timestamp` 컬럼의 타임존)

무대 쪽:

- **미루는 코드나 버리는 호출은 검사가 배웅까지 한다** — 남은 대기가 다음 테스트의
  무대 위에서 돌거나, 빨간불이 아니라 「jest가 안 닫힌다」로 끝난다
- **무대를 조종했으면 되돌리는 자리까지 만든다** — 끊긴 클라이언트의 teardown은
  **정지**로 끝나 원인이 안 읽힌다. 그래서 끊긴 클라이언트로 사후 정리를 재는
  검사는 만들지 않고, 그 입력을 직접 먹이는 단위 검사로 간다
- **무대가 하나면 스펙끼리 전제를 무너뜨린다** — 같은 대회를 쓰지 않는다
...
4. 같은 브랜치에 SSOT 커밋을 얹는다 — 결함 대장의 상태 열,
   `backlog.md`의 방향, `domain.md`의 규칙, 이 파일의 기준선
...
예외는 `docs/archive/`다. **기록물이라 갱신하지 않는다** —
...
근거: T29에서 태스크 리뷰 둘이 16분을 쓰고 아무것도 못 잡았고, 최종 전체 리뷰
하나가 Important 3건을 혼자 다 잡았다. 태스크 경계의 기준은 **리뷰어가 이웃
태스크는 통과시키면서 이것만 반려할 수 있는가**다.
