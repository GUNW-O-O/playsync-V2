...
아니다. 프론트가 187에서 189로 는 것은 T84가 콘솔의 등록 배지에 회귀 둘을
붙였기 때문이다. **파일 수는 32가 아니라 31이었다** — #96의 기준선이 그
자리에서 틀렸고, `main`을 세어 확인했다.

e2e · 촬영 · 부하 하네스는 CI가 아니라 사람이 돌린다. **정산 촬영은 마무리
셋(`DEMO_ENDING`)을 각각 한 번씩 돌린다** — 하나가 대회를 닫으면 나머지는
찍을 자리가 없어서고, 그래서 매번 시드를 다시 깐다. 셋 다 12~13분이고 그중
10분은 등록 마감을 기다리는 시간이라 영상에서 잘려 나간다.

**e2e는 T84에서 다시 돌리지 않았다** — 회귀 프로젝트(`e2e/*.spec.ts`)가 보는
화면이 바뀌지 않았다. 바뀐 것은 상점 콘솔의 등록 배지 하나이고 그것은 프론트
단위가 든다. 대신 정산 촬영 셋이 실제로 돌았다.
