 ...
 *
 * 5분은 그 사이다. 실측으로 **리바인이 +1분 50초**, 파이널 테이블이 서는 것이
 * **+3분 29초**다. 리바인까지 2.7배 여유가 있다.
 *
 * **10분이었다가 줄였다.** 그 값은 판을 테이블마다 차례로 돌리던 때의 실측
 * (리바인 +2분 10초)에 여유를 얹은 것인데, 그 뒤 첫 판·둘째 판을 한꺼번에
 * 돌리게 되면서 앞이 더 빨라졌다. 창은 그대로여서 **파이널이 선 뒤 6분 28초를
 * 그냥 기다렸다** — 잘려 나가는 구간이지만 촬영 한 번이 그만큼 길어지고, 셋을
 * 찍으므로 세 배로 든다.
 *
 * 3~4분까지도 되지만 안 잡는다. 실패의 값이 다르기 때문이다 — 마감이 일찍
 * 오면 리바인을 못 물어 **그 장면 자체가 사라지고 촬영을 다시 돌려야** 한다.
 * 기다리는 것은 시간만 든다.
 *
 * **마감 전이어야 하는 것은 리바인뿐이다.** 파이널 도달이 5분을 넘어도 무해한데,
 * 병합은 `releaseSeats`가 `WAITING`만 요구하고 마감을 요구하는 것은
 * `isFinalTable` 하나이기 때문이다.
 *
