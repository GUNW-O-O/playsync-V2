...
/**
 * 찍는 주기. **이 값이 곧 정지 시간의 오차 상한이다.**
 *
 * 복구는 "마지막 하트비트부터 지금까지"를 정지로 계산하므로, 실제 정지보다
 * 최대 이 값만큼 크게 잡는다. 그 초과분은 `pausedMs`에 그대로 쌓이고
 * 블라인드 레벨이 그만큼 늦게 오른다.
 *
 * **짧은 재시작이 가장 크게 틀린다.** 30초 주기였을 때 5초짜리 배포 재시작이
 * 최대 35초로 계상됐다 — 7배다. 대회 한 판에 재시작이 몇 번 있으면 블라인드
 * 일정이 분 단위로 밀린다. 긴 장애에서는 비율이 작아 눈에 안 띄는데, 흔한
 * 것은 짧은 쪽이다.
 *
 * 5초면 같은 재시작의 오차가 5초로 떨어진다. 대가는 싱글턴 행 하나에 대한
 * upsert가 분당 2회에서 12회로 느는 것뿐이다.
 */
 ...
 * 근거다.
 *
 * **주기가 곧 정지 시간의 오차 상한이다**(`DEFAULT_INTERVAL_MS`). 그 오차는
 * 한 방향으로만 틀린다 — 정지를 실제보다 길게 잡아 블라인드가 늦게 오른다.
 *
