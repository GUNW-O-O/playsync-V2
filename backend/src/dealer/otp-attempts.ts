...
export class OtpAttempts {
   ...
   *
   * - 여섯 대가 같은 대회에 한꺼번에 인증하면 INCR이 1~6이 되고, 여섯 번째는
   *   OTP가 맞는데도 403을 받는다. 게이트가 bcrypt 앞이라 생기는 값이고, 앞이
   *   아니면 스레드풀을 못 지킨다. 실무 영향은 작다 — 앞선 성공이 `clear`를
   *   부르므로 한 번 다시 누르면 통과한다.
   * - OTP는 맞았는데 대회가 `FINISHED`거나 딜러 세션이 없어 던지는 경로는
   *   슬롯을 쓰고 `clear`를 부르지 않는다. 여섯 번째 재시도부터 안내 문구가
   *   "시도가 너무 많습니다"로 바뀌어, 준비가 덜 됐다는 진짜 원인을 가린다.
   *
   * 둘 다 가용성 문제이고 보안 영향은 없다. `backlog.md`의 이월 항목이다.
   */
