...
export class TableEngine {
  ...
  public startPreFlop() {
    ...
    //
    // state.ante는 이미 금액이다(0이면 없음) — DealerService.startPreFlop과
    // RecoveryService가 deriveAnteAmount 하나로 만들어 싣는다. sb / 5 같은
    // 식을 여기서 다시 계산하지 않는다(T58).
    if (this.state.ante > 0) {
      this.payAnte(this.state.ante);
    }
