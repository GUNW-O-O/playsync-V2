...
export interface TableState {
   ...
   *
   * **이 필드가 서 있는 동안 `actionDeadline`은 없다.** 마감을 그대로 두면
   * 돌아온 사람이 누른 버튼이 시간 초과로 바뀐다(T94의 두 경로 중 하나).
   * 그래서 "마감이 없다"의 뜻이 둘로 갈리는데 — 차례가 없거나(쇼다운·대기),
   * 멈춰 있거나 — 이 필드가 그 둘을 가른다.
   *
   * 계약에도 있다(`TableStateSchema`). 이 사건을 겪은 단말은 **전부 재접속한
   * 단말**이라 지나간 이벤트를 받을 수 없고, 스냅샷만이 접속 시점에 한 번
   * 전달된다(`WsGateway.handleConnection`).
   */
  resumePending?: {
