 ...
 * 재개 유예의 기본값.
 *
 * **남은 시간을 복원하려 들지 않는다.** 하트비트를 5초로 줄여도(T94) "언제
 * 죽었나"에는 여전히 그만큼의 해상도가 있고, 무엇보다 **그게 맞는 일인지가
 * 의심스럽다** — 몇 분간 깜깜했던 화면 앞에 돌아온 사람에게 남은 4초를 돌려
 * 주는 것은 산술적으로만 공정하다. 턴을 통째로 다시 주면 틀리는 방향이
 * 한쪽(사람에게 유리)으로 고정되고, 그 방향이면 아무도 돈을 잃지 않는다.
 *
