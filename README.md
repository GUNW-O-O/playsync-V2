...
- **실시간 게임 로직의 테이블별 상태 정합성 보장**
  ...
  - 서버 타이머에 의존하는 대신 Lazy Update 방식을 채택하여 핸드 시작 시점에만 레벨을 계산하도록 설계했습니다. 이를 통해 서버의 유효 상태(State) 관리를 최소화하여 확장성 있는 Stateless 구조를 구현했습니다.
...
- **테이블 단위 액션 타임아웃 관리**
  ...
  - BullMQ 지연 잡의 `jobId`를 테이블 ID로 고정하여, 테이블당 활성 타이머가 항상 하나만 존재하도록 설계했습니다. 새 턴 시작 시 기존 잡을 제거하고 새 잡으로 교체하며, 타임아웃 발생 시점에 현재 턴 유저가 여전히 동일한지 재검증 후 자동 액션을 실행합니다.
...
- 사용자수가 많을것으로 가정한 효율 중심의 아키텍처 설계를 진행했습니다.
  ...
  - 결과 정산 시점에만 DB 트랜잭션을 실행하는 Write-back 패턴을 적용했습니다. 핸드 진행 중 DB 쓰기는 0회 — 핸드당 수십 회 발생하는 상태 변경은 전부 Redis에서 처리되고, DB에는 정산·탈락 등 경계 이벤트에서 1~2회만 기록됩니다.
